  ...
  it("seeds users-linked rows and asserts they survive every migration", () => {
    ...
    for (const file of sqlFiles) {
      ...
      if (!seeded && file.startsWith("0000_")) {
        ...
        db.exec(
          `INSERT INTO users (id, username) VALUES ('u-eval', 'eval_user')`,
        );
        db.exec(
    ...
    const accounts = db
      .query(`SELECT id FROM account WHERE id = 'acct-eval'`)
      .all();
    expect(accounts).toHaveLength(1);

    const sessions = db
      .query(`SELECT id FROM sessions WHERE id = 'sess-eval'`)
      .all();
  ...
  it("seeds titles-linked rows and asserts they survive every migration", () => {
    ...
    for (const file of sqlFiles) {
      ...
      if (!seededUser && file.startsWith("0000_")) {
        db.exec(
          `INSERT INTO users (id, username) VALUES ('u-t', 'title_user')`,
        );
        seededUser = true;
      }
    ...
    const titles = db
      .query(`SELECT id FROM titles WHERE id = 'title-eval'`)
      .all();
