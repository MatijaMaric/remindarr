...
function loadOrCreateExpected(name: string, actual: unknown): { matched: boolean; diff?: string } {
...
describe("parseMovieDetails golden set", () => {
  it("standard movie with watch providers", () => {
    ...
    const { matched, diff } = loadOrCreateExpected("movie-with-providers.parsed", result)
    if (!matched) {
  ...
  it("movie without watch providers", () => {
    ...
    const { matched, diff } = loadOrCreateExpected("movie-no-providers.parsed", result)
    if (!matched) {
      throw new Error(`TMDB parser output drifted from golden set:\n${diff}`)
    }
  })
  ...
  it("movie with null/missing optional fields", () => {
    ...
    const { matched, diff } = loadOrCreateExpected("movie-sparse.parsed", result)
    if (!matched) {
      throw new Error(`TMDB parser output drifted from golden set:\n${diff}`)
    }
  })
...
describe("parseTvDetails golden set", () => {
  it("standard show with watch providers", () => {
    ...
    const { matched, diff } = loadOrCreateExpected("show-with-providers.parsed", result)
    if (!matched) {
      throw new Error(`TMDB parser output drifted from golden set:\n${diff}`)
    }
  })
  ...
  it("show with dedup logic: FLATRATE beats FREE beats ADS for same provider", () => {
    ...
    for (const offer of result.offers) {
      ...
      providerCounts.set(offer.providerId, [...existing, offer.monetizationType])
    }

    for (const [, types] of providerCounts) {
      if (types.length > 1) {
        // Shouldn't happen — dedup removes lower-priority types for same provider
        throw new Error(`Provider appears with multiple monetization types: ${types.join(", ")}`)
      }
    }

    const { matched, diff } = loadOrCreateExpected("show-provider-dedup.parsed", result)
    if (!matched) {
      throw new Error(`TMDB parser output drifted from golden set:\n${diff}`)
    }
  })
