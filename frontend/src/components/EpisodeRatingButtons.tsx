...
export default function EpisodeRatingButtons({
  episodeId,
}: EpisodeRatingButtonsProps) {
  const { user } = useAuth();
  ...
  if (isLoading) {
    return (
      <div
        ...
        data-testid="episode-rating-loading"
      >
...
function EpisodeFriendsRatings({
  ratings,
}: {
  ratings: EpisodeRatingResponse["friends_ratings"];
}) {
