...
describe("classifySession", () => {
  ...
  it("classifies a 503 server error as indeterminate", () => {
    expect(classifySession({ data: null, error: { status: 503 } }, false)).toBe(
      "indeterminate",
    );
  });

  it("classifies a status-0 network error as indeterminate", () => {
    expect(classifySession({ data: null, error: { status: 0 } }, false)).toBe(
      "indeterminate",
    );
  });

  it("classifies an error with no status as indeterminate", () => {
    expect(classifySession({ data: null, error: {} }, false)).toBe(
      "indeterminate",
    );
  });

  it("classifies a 401 as unauthenticated (definitive)", () => {
    expect(classifySession({ data: null, error: { status: 401 } }, false)).toBe(
      "unauthenticated",
    );
  });

  it("classifies a 403 as unauthenticated (definitive)", () => {
    expect(classifySession({ data: null, error: { status: 403 } }, false)).toBe(
      "unauthenticated",
    );
  });

  it("classifies null data with no error as unauthenticated", () => {
    expect(classifySession({ data: null, error: null }, false)).toBe(
      "unauthenticated",
    );
  });
  ...
  it("classifies a result with user data as authenticated", () => {
    expect(
      classifySession({ data: { user: { id: "u1" } }, error: null }, false),
    ).toBe("authenticated");
  });
});
