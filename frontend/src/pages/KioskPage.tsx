...
function StripePoster({
  ...
  const bg =
    tone === "paper" ? `oklch(0.92 0.03 ${hue})` : `oklch(0.28 0.04 ${hue})`;
  const stripe =
    tone === "paper" ? `oklch(0.88 0.04 ${hue})` : `oklch(0.24 0.05 ${hue})`;
  const fg =
    tone === "paper" ? `oklch(0.35 0.05 ${hue})` : `oklch(0.78 0.08 ${hue})`;
  return (
    <div
      style={{
        width: "100%",
...
function CastIcon({
  ...
  return (
    ...
    >
      ...
      />
      <path
        d="M1.5 13 q1.5 0 1.5 1.5"
        stroke={color}
        strokeWidth="1.5"
        strokeLinecap="round"
      />
      <path
        d="M1.5 10.5 q4 0 4 4"
        stroke={color}
        strokeWidth="1.5"
        strokeLinecap="round"
      />
      <path
        d="M1.5 8 q6.5 0 6.5 6.5"
        stroke={color}
        strokeWidth="1.5"
        strokeLinecap="round"
      />
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <div
          style={{
            width: 30,
            height: 30,
            borderRadius: epaper ? 0 : 7,
            background: epaper ? C.bg : "#fbbf24",
            color: epaper ? C.text : "#000",
            display: "flex",
            alignItems: "center",
        ...
        <div
          style={{
            ...
            padding: "3px 8px",
            borderRadius: epaper ? 0 : 4,
            background: epaper ? "transparent" : "rgba(255,255,255,0.04)",
            border: epaper
              ? "1px solid rgba(246,243,232,0.5)"
              : "1px solid rgba(255,255,255,0.06)",
            textTransform: "uppercase",
          }}
        >
          KIOSK · {fidelity.toUpperCase()}
        </div>
      </div>
      <div style={{ flex: 1 }} />
      <div
        style={{
          ...Mono,
          fontSize: 13,
          color: epaper ? "rgba(246,243,232,0.85)" : C.dim,
          letterSpacing: 1.5,
        }}
      >
        {household}
...
function PanelHeader({
  ...
  return (
    ...
    >
      ...
      <div
        style={{ ...Mono, fontSize: 11, color: C.veryDim, letterSpacing: 1.5 }}
      >
        {right}
      </div>
    </div>
...
function ReleasingTodayPanel({
  ...
  return (
    <Panel C={C} epaper={epaper}>
      <PanelHeader
        C={C}
        kicker="Releasing today"
        right={`${releases.length} drops`}
      />
      <div
        style={{
          display: "flex",
            ...
            return (
              <div
                ...
                style={{
                  ...
                  padding: "12px 16px",
                  borderBottom:
                    i < releases.slice(0, 8).length - 1
                      ? `1px solid ${C.borderSoft}`
                      : "none",
                  opacity: isPast ? (epaper ? 0.55 : 0.6) : 1,
                  animation: breathe
                    ? `kfade 0.5s ease ${0.05 * i}s both`
                    : "none",
                }}
              >
                <div
                ...
                >
                  {r.air_date
                    ? new Date(r.air_date).toLocaleTimeString(undefined, {
                        hour: "2-digit",
                        minute: "2-digit",
                        hour12: false,
                      })
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  >
                    ...
                    {r.provider && (
                      <>
                        · <span style={{ color: C.dim }}>{r.provider}</span>
                      </>
                    )}
                  </div>
                </div>
                    ...
                    background:
                      r.kind === "series"
                        ...
                        : epaper
                          ...
                          : C.chip,
                    color:
                      r.kind === "series" ? (epaper ? C.bg : C.accent) : C.dim,
                    border: epaper ? `1px solid ${C.text}` : "none",
                    flexShrink: 0,
                  }}
                >
              ...
              >
                ...
                <div
                  style={{
                    ...
                    fontSize: 14,
                    fontWeight: 800,
                    color: q.left > 1 ? C.accent : C.text,
                    minWidth: 30,
                    textAlign: "right",
                    flexShrink: 0,
                  }}
...
export default function KioskPage() {
  ...
  const fidelity: KioskFidelity = FIDELITY_VALUES.includes(
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const params =
        fidelity !== "rich" ? `?display=${encodeURIComponent(fidelity)}` : "";
      const tz = Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC";
      const res = await fetch(
        `/api/kiosk/${encodeURIComponent(token!)}${params}`,
        {
      ...
      >
        <div style={{ textAlign: "center", maxWidth: 360, padding: "0 24px" }}>
          ...
          <p style={{ color: "#a1a1aa", fontSize: 14 }}>
            This kiosk link is no longer valid. Ask the owner to share a new
            one.
          </p>
        </div>
      </div>
  ...
  const refreshSec =
    data?.meta.refresh_interval_seconds ?? (fidelity === "epaper" ? 1800 : 300);
  const refreshLabel =
    refreshSec >= 1800
      ? `Auto-refreshes every ${Math.round(refreshSec / 60)} min`
    ...
    >
      ...
      {data?.airing_now ? (
        ...
        <HeroFeatured
          ...
          epaper={epaper}
          breathe={breathe}
          item={heroFallbackItem}
          kicker={heroFallbackKicker}
        />
      ) : (
      ...
      >
        ...
        <span>
          {typeof window !== "undefined" ? window.location.hostname : ""}/kiosk
        </span>
      </div>
    </div>
  );
