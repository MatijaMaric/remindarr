 ...
 */
export async function evaluateCountMovies(
  userId: string,
  threshold: number,
): Promise<EvalResult> {
  return traceDbQuery("evaluateCountMovies", async () => {
    ...
    const row = await db
      ...
      .where(
        and(eq(watchedTitles.userId, userId), eq(titles.objectType, "MOVIE")),
      )
      .get();
 ...
 */
export async function evaluateCountEpisodes(
  userId: string,
  threshold: number,
): Promise<EvalResult> {
 ...
 */
export async function evaluateGenreCount(
  userId: string,
  threshold: number,
  genre: string,
): Promise<EvalResult> {
  return traceDbQuery("evaluateGenreCount", async () => {
    ...
    if (genre === "__any__") {
      ...
      const row = await db
        ...
        .innerJoin(titleGenres, eq(titleGenres.titleId, watchedTitles.titleId))
        .where(
          and(eq(watchedTitles.userId, userId), eq(titleGenres.genre, genre)),
        )
        .get();
      progress = row?.cnt ?? 0;
    }
 ...
 */
export async function evaluateCompletionist(
  userId: string,
  threshold: number,
  titleId?: string,
): Promise<EvalResult> {
 ...
 */
export async function evaluateSocialFirstRecommendation(
  userId: string,
): Promise<EvalResult> {
 ...
 */
export async function evaluateSocialFirstFollow(
  userId: string,
): Promise<EvalResult> {
 ...
 */
export async function evaluateDecadeCount(
  userId: string,
  threshold: number,
): Promise<EvalResult> {
  return traceDbQuery("evaluateDecadeCount", async () => {
    ...
    const row = await db
      .select({
        cnt: sql<number>`COUNT(DISTINCT CAST(${titles.releaseYear} / 10 AS INTEGER))`,
      })
      .from(watchedTitles)
      .innerJoin(titles, eq(titles.id, watchedTitles.titleId))
      .where(
        and(
          ...
          sql`${titles.releaseYear} IS NOT NULL`,
        ),
      )
      .get();
    const progress = row?.cnt ?? 0;
 ...
 */
export async function evaluateLanguageCount(
  userId: string,
  threshold: number,
): Promise<EvalResult> {
  return traceDbQuery("evaluateLanguageCount", async () => {
    ...
    const row = await db
      ...
      .where(
        and(
          eq(watchedTitles.userId, userId),
          sql`${titles.originalLanguage} IS NOT NULL`,
        ),
      )
      .get();
    const progress = row?.cnt ?? 0;
 ...
 */
export async function evaluateMiniseriesCompleted(
  userId: string,
  threshold: number,
): Promise<EvalResult> {
 ...
 */
export async function evaluateDeepShowCompleted(
  userId: string,
  threshold: number,
): Promise<EvalResult> {
