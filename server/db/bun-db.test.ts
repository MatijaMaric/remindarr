...
describe("fixSkippedMigrations", () => {
  ...
  test("applies skipped migrations when title_genres is missing", () => {
    ...
    `);
    const hash0000 = crypto.createHash("sha256").update(m0000sql).digest("hex");
    const hash0001 = crypto.createHash("sha256").update(m0001sql).digest("hex");
