...
export async function deleteRecommendation(id: string, userId: string) {
  return traceDbQuery("deleteRecommendation", async () => {
    ...
    await db
      ...
      .where(
        and(eq(recommendations.id, id), eq(recommendations.fromUserId, userId)),
      )
      .run();
  });
}
...
export async function getUnreadCount(userId: string): Promise<number> {
  return traceDbQuery("getUnreadRecommendationCount", async () => {
    ...
    const row = await db
      ...
      .where(
        and(visibilityCondition, sql`${recommendationReads.readAt} IS NULL`),
      )
      .get();
    return row?.count ?? 0;
  });
