...
describe("getWatchHistoryById", () => {
  ...
  it("returns the row for the correct user", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "movie-ghb-1", objectType: "MOVIE" }),
    ]);
  ...
  it("returns null for another user's row id", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "movie-ghb-2", objectType: "MOVIE" }),
    ]);
...
describe("updateWatchHistoryWatchedAt", () => {
  it("updates the watched_at value for the correct row", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "movie-upd-1", objectType: "MOVIE" }),
    ]);
    ...
    await updateWatchHistoryWatchedAt(
      histRow.id,
      userId,
      "2020-06-15 12:00:00",
    );
  ...
  it("does not affect rows belonging to another user", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "movie-upd-2", objectType: "MOVIE" }),
    ]);
    await logWatch(otherUserId, "movie-upd-2");

    const db = getRawDb();
    const histRow = db
      .prepare("SELECT id, watched_at FROM watch_history WHERE user_id = ?")
      .get(otherUserId) as { id: string; watched_at: string };
...
describe("getLatestWatchHistoryFor", () => {
  it("returns null when no history rows exist", async () => {
    const result = await getLatestWatchHistoryFor(
      userId,
      "nonexistent-title",
      null,
    );
  ...
  it("returns the maximum watched_at for a movie (episodeId = null)", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "movie-lat-1", objectType: "MOVIE" }),
    ]);
  ...
  it("returns the maximum watched_at for a specific episode", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "show-lat-1", objectType: "SHOW" }),
    ]);
  ...
  it("does not mix movie and episode rows when episodeId is null", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "show-lat-2", objectType: "SHOW" }),
    ]);
    ...
    const movieResult = await getLatestWatchHistoryFor(
      userId,
      "show-lat-2",
      null,
    );
...
describe("getTitleWatchHistory", () => {
  it("default: returns { history, has_more: false, next_cursor: null } for result within default limit", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "movie-twh-1", objectType: "MOVIE" }),
    ]);
  ...
  it("returns has_more: true and non-null next_cursor when more rows exist", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "movie-twh-2", objectType: "MOVIE" }),
    ]);
    ...
    const result = await getTitleWatchHistory(userId, "movie-twh-2", {
      limit: 2,
    });
  ...
  it("cursor advances correctly with no overlap or gap", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "movie-twh-3", objectType: "MOVIE" }),
    ]);
    const db = getRawDb();
    ...
    db.run(
      ...
      [randomUUID(), userId, "movie-twh-3", null, "2024-01-01 10:00:00", null],
    );
    db.run(
      "INSERT INTO watch_history (id, user_id, title_id, episode_id, watched_at, note) VALUES (?, ?, ?, ?, ?, ?)",
      [randomUUID(), userId, "movie-twh-3", null, "2024-01-02 10:00:00", null],
    );
    db.run(
      "INSERT INTO watch_history (id, user_id, title_id, episode_id, watched_at, note) VALUES (?, ?, ?, ?, ?, ?)",
      [randomUUID(), userId, "movie-twh-3", null, "2024-01-03 10:00:00", null],
    );
    db.run(
      "INSERT INTO watch_history (id, user_id, title_id, episode_id, watched_at, note) VALUES (?, ?, ?, ?, ?, ?)",
      [randomUUID(), userId, "movie-twh-3", null, "2024-01-04 10:00:00", null],
    );

    const page1 = await getTitleWatchHistory(userId, "movie-twh-3", {
      limit: 2,
    });
    ...
    const page2 = await getTitleWatchHistory(userId, "movie-twh-3", {
      limit: 2,
  ...
  it("identical-timestamp keyset correctness: no dup or skip on tie-break", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "movie-twh-4", objectType: "MOVIE" }),
    ]);
    const db = getRawDb();
    ...
    db.run(
      "INSERT INTO watch_history (id, user_id, title_id, episode_id, watched_at, note) VALUES (?, ?, ?, ?, ?, ?)",
      [randomUUID(), userId, "movie-twh-4", null, sameTs, null],
    );
    db.run(
      "INSERT INTO watch_history (id, user_id, title_id, episode_id, watched_at, note) VALUES (?, ?, ?, ?, ?, ?)",
      [randomUUID(), userId, "movie-twh-4", null, sameTs, null],
    ...
    const page1 = await getTitleWatchHistory(userId, "movie-twh-4", {
      limit: 2,
    });
    expect(page1.history.length).toBe(2);
    expect(page1.has_more).toBe(true);
    ...
    const page2 = await getTitleWatchHistory(userId, "movie-twh-4", {
      limit: 2,
      before: page1.next_cursor!,
    });
  ...
  it("episodeId filter: returns only matching episode rows", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "show-twh-5", objectType: "SHOW" }),
    ]);
    ...
    const result = await getTitleWatchHistory(userId, "show-twh-5", {
      episodeId: epId,
    });
  ...
  it("episodeId absent: returns all rows", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "show-twh-6", objectType: "SHOW" }),
    ]);
  ...
  it("limit clamping: caps at 100 rows max", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "movie-twh-7", objectType: "MOVIE" }),
    ]);
    const db = getRawDb();
    ...
    for (let i = 0; i < 102; i++) {
      ...
      db.run(
        ...
        [
          ...
          "movie-twh-7",
          null,
          `2024-01-${day} ${hour}:00:00`,
          null,
        ],
      );
    }

    const result = await getTitleWatchHistory(userId, "movie-twh-7", {
      limit: 200,
    });
  ...
  it("malformed cursor is safely ignored and returns first-page results", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "movie-twh-8", objectType: "MOVIE" }),
    ]);
    ...
    const result = await getTitleWatchHistory(userId, "movie-twh-8", {
      before: "nocursorpipe",
    });
    expect(result.history.length).toBe(2);
  ...
  it("next_cursor is null on the final page", async () => {
    await upsertTitles([
      makeParsedTitle({ id: "movie-twh-9", objectType: "MOVIE" }),
    ]);
    const db = getRawDb();
    // Insert 5 rows
    for (let i = 0; i < 5; i++) {
    ...
    let pages = 0;
    do {
      lastResult = await getTitleWatchHistory(userId, "movie-twh-9", {
        limit: 2,
