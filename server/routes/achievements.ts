...
achievementsApp.get(
  "/u/:username",
  requireAuth,
  zValidator("param", usernameSchema),
  async (c) => {
...
achievementsApp.get(
  ...
  async (c) => {
    ...
    if (meta?.family) {
      const familyAchievements = ACHIEVEMENTS.filter((a) => {
      ...
      const earnedKeys = new Set(
        userRows.filter((r) => r.earnedAt != null).map((r) => r.achievementKey),
      );
      ladder = {
        rungs: familyAchievements.map((a) => ({
          ...
          earnedAt:
            userRows.find((r) => r.achievementKey === a.key)?.earnedAt ?? null,
        })),
      };
    }
    ...
    const rarity =
      meta?.tier === "one-shot" ? await getRarityForKey(key) : null;
...
achievementsApp.get(
  "/u/:username/:key",
  requireAuth,
  zValidator("param", usernameKeySchema),
  async (c) => {
    const requester = c.get("user")!;
    ...
    if (
      profileUser.visibility === "friends_only" &&
      profileUser.id !== requester.id
    ) {
      const following = await isFollowing(requester.id, profileUser.id);
    ...
    const row = userRows.find(
      (r) => r.achievementKey === key && r.earnedAt != null,
    );
