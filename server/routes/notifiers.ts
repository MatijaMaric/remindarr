...
const quietHoursTimeSchema = z
  .string()
  .regex(HHMM, { message: "Invalid time format. Use HH:MM (24h)" });
...
const createNotifierSchema = z
  ...
  .refine(
    (v) =>
      v.digest_mode !== "weekly" ||
      (v.digest_day !== undefined && v.digest_day !== null),
    {
...
app.post("/", zValidator("json", createNotifierSchema), async (c) => {
  ...
  if (!providerImpl) {
    return err(
      c,
      `Unknown provider: ${provider}. Available: ${getAvailableProviders().join(", ")}`,
    );
  }
