...
async function getEpisodeId(
  ...
  if (!row)
    throw new Error(`Episode not found: ${titleId} s${season}e${episode}`);
  return row.id;
}
...
describe("GET /up-next", () => {
  ...
  it("returns in_progress item when show has been partially watched", async () => {
    ...
    const body = (await res.json()) as {
      items: Array<{ kind: string; titleId: number; title: string }>;
    };
  ...
  it("returns newly_aired item when show has unwatched aired episodes and was never started", async () => {
    ...
    expect(res.status).toBe(200);
    const body = (await res.json()) as {
      items: Array<{ kind: string; titleId: number }>;
    };
